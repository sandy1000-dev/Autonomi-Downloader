#!/usr/bin/env python
"""gRPC integration test for antd Python SDK.

Mirrors antd-grpc-test/Program.cs -- standalone script with colored pass/fail output.
Requires a running antd daemon with gRPC on localhost:50051.

Usage: python scripts/test_grpc.py
"""
...
def main():
    ...
    print(f"\n{BOLD}{CYAN}antd Python SDK - gRPC Integration Test{RESET}")
    print(f"Target: {target}\n")

    client = AntdClient(transport="grpc", target=target)

    # 1. Health check (via data cost as a connectivity test)
    try:
        status = client.health()
        if status.ok:
            test_pass(f"Health check (gRPC, network={status.network})")
        else:
            test_fail("Health check (gRPC)")
            print(f"\n{RED}Cannot reach antd gRPC at {target}. Is it running?{RESET}")
            return 1
    except Exception as e:
        test_fail("Health check (gRPC)", str(e))
        print(f"\n{RED}Cannot reach antd gRPC at {target}. Is it running?{RESET}")
        return 1
    ...
    data_addr = None
    try:
        test_data = b"hello from antd python SDK (gRPC)!"
    ...
    if data_addr:
        try:
            ...
            if got == b"hello from antd python SDK (gRPC)!":
    ...
    try:
        cost = client.data_cost(b"cost estimation test data (gRPC)")
    ...
    chunk_addr = None
    try:
        chunk_data = b"chunk test payload from python gRPC"
    ...
    if chunk_addr:
        try:
            ...
            if got == b"chunk test payload from python gRPC":
