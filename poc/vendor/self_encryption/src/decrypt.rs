...
pub fn decrypt_sorted_set(
    ...
    // Decrypt chunks (in parallel on native platforms), then concatenate in order.
    #[cfg(not(target_arch = "wasm32"))]
    let decrypted_chunks: Vec<Bytes> = {
        encrypted_chunks
            .par_iter()
            .enumerate()
    ...
    // Single-threaded fallback for wasm32 (no OS threads available).
    #[cfg(target_arch = "wasm32")]
    let decrypted_chunks: Vec<Bytes> = {
        let mut v = Vec::with_capacity(encrypted_chunks.len());
        for (chunk_index, chunk) in encrypted_chunks.iter().enumerate() {
            v.push(decrypt_chunk(
                chunk_index,
                &chunk.content,
                &src_hashes,
                child_level,
